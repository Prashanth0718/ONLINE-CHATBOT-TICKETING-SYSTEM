...
const UserManagement = () => {
  ...
  const fetchUserBookings = async (userId) => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.get(`${backendURL}/api/admin/users/${userId}/bookings`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const resetPassword = async (id) => {
    try {
      const token = localStorage.getItem("token");
      await axios.post(`${backendURL}/api/admin/users/reset-password/${id}`, {}, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleSaveEdit = async () => {
    const token = localStorage.getItem("token");
    try {
      await axios.put(`${backendURL}/api/admin/bookings/${editingBooking._id}`, editedData, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleDeleteBooking = async (bookingId) => {
    ...
    try {
      await axios.delete(`${backendURL}/api/admin/bookings/${bookingId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
